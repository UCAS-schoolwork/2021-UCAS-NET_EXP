...
#define tcp_tlock               tcp_sock_table.tlock
#define tcp_bind_tlock          tcp_sock_table.bind_tlock
...
void init_tcp_stack()
{
	...
	pthread_mutex_init(&tcp_tlock,NULL);
	pthread_mutex_init(&tcp_bind_tlock,NULL);
	srand(time(NULL));
	pthread_t timer;
...
struct tcp_sock *alloc_tcp_sock()
{
	...
	tsk->snd_wnd = 1*MSS;
	tsk->c_counter = 0;
	tsk->ssthresh = TCP_DEFAULT_WINDOW/1500;
...
#define MAXSOCK 30
int sock_used = 0;
struct tcp_sock *tsks[MAXSOCK];
int mysocket(int domain, int type, int protocol)
{
	if(++sock_used>=MAXSOCK) 
		return -1;
	return sock_used;
}
int mybind(int fd, struct sockaddr * addr, socklen_t len)
{
	struct tcp_sock* tsk = alloc_tcp_sock();
	tsks[fd] = tsk;
	struct sock_addr skaddr;
	skaddr.ip = ((struct sockaddr_in*)addr)->sin_addr.s_addr;
	skaddr.port = ((struct sockaddr_in*)addr)->sin_port;
	return tcp_sock_bind(tsk,&skaddr);
}

int mylisten(int fd, int n)
{
	assert(fd>0 && fd<MAXSOCK);
	struct tcp_sock* tsk = tsks[fd];
	return tcp_sock_listen(tsk,n);
}

int myaccept(int fd,struct sockaddr* addr, socklen_t *len)
{
	struct tcp_sock* tsk = tsks[fd];
...
int myrecv(int fd, void* buf, size_t n, int flags)
{
	struct tcp_sock* tsk = tsks[fd];
	return tcp_sock_read(tsk, buf, n);
}

int mysend(int fd, void* buf, size_t n, int flags)
{
	struct tcp_sock* tsk = tsks[fd];
	return tcp_sock_write(tsk,buf,n);
}
...
void free_tcp_sock(struct tcp_sock *tsk)
{
	...
	if(--tsk->ref_cnt<=0){
		...
		if(!list_empty(&tsk->accept_queue)) log(ERROR, "free_tcp accept_queue.");
		...
		tcp_send_buf_free(tsk);
		tcp_ofo_buf_free(tsk);
...
struct tcp_sock *tcp_sock_lookup_established(u32 saddr, u32 daddr, u16 sport, u16 dport)
{
	...
	pthread_mutex_lock(&tcp_tlock);
	list_for_each_entry(tsk,&tcp_established_sock_table[id],hash_list){
		if(tsk->sk_sip==saddr && tsk->sk_sport==sport && tsk->sk_dip==daddr && tsk->sk_dport==dport){
			pthread_mutex_unlock(&tcp_tlock);
			return tsk;
		}
	}
	pthread_mutex_unlock(&tcp_tlock);
...
struct tcp_sock *tcp_sock_lookup_listen(u32 saddr, u16 sport)
{
	//fprintf(stdout, "TODO: implement %s please.\n", __FUNCTION__);
	saddr = 0;
	int id = tcp_hash_function(saddr,0,sport,0);
	struct tcp_sock *tsk = NULL;
	pthread_mutex_lock(&tcp_tlock);
	list_for_each_entry(tsk,&tcp_listen_sock_table[id],hash_list){
...
{
	int bind_hash_value = tcp_hash_function(tsk->sk_sip, 0, tsk->sk_sport, 0);
	struct list_head *list = &tcp_bind_sock_table[bind_hash_value];
	pthread_mutex_lock(&tcp_bind_tlock);
	list_add_head(&tsk->bind_hash_list, list);
	tsk->ref_cnt += 1;
	pthread_mutex_unlock(&tcp_bind_tlock);
	return 0;
}
...
static int tcp_port_in_use(u32 saddr,u16 sport)
{
	int value = tcp_hash_function(saddr, 0, sport, 0);
...
static u16 tcp_get_port(u32 saddr)
{
	for (u16 port = PORT_MIN; port < PORT_MAX; port++) {
		if (!tcp_port_in_use(saddr,port))
			return port;
	}
	return 0;
}
...
{
	if ((port && tcp_port_in_use(tsk->sk_sip,port)) ||
			(!port && !(port = tcp_get_port(tsk->sk_sip))))
		return -1;
	tsk->sk_sport = port;
	return 0;
}
...
int tcp_hash(struct tcp_sock *tsk)
{
	...
	if (tsk->state == TCP_LISTEN) {
		hash = tcp_hash_function(tsk->sk_sip, 0, tsk->sk_sport, 0);
		list = &tcp_listen_sock_table[hash];
	}
	else { //not only established, but all tsk that has 4-tuple.
...
inline int tcp_is_allow_to_send(struct tcp_sock *tsk)
{
	return less_than_32b(tsk->snd_nxt,tsk->snd_una+tsk->snd_wnd+MSS*tsk->dup_ack);
}
...
int tcp_sock_write(struct tcp_sock *tsk, char *buf, int len)
{
	...
	while(len>0){
		...
		while(!tcp_is_allow_to_send(tsk)){
			//printf("send window full. sleep. %u %u %u %u\n",tsk->snd_nxt,tsk->snd_una,tsk->snd_wnd,tsk->dup_ack*MSS);
			sleep_with_lock(tsk->wait_send);
		}
		//sendlen = min(sendlen,(int)(tsk->snd_una+tsk->snd_wnd-tsk->snd_nxt));
