...
#define MIN(x,y) (x<y?x:y)
		...
		ret = mysend(sockid, fcache[sv->fidx].file + sv->total_sent, len, 0);
		if (ret < 0) {
...
int handle_connection(int sockid, struct server_vars *sv)
{
	...
	int rd = myrecv(sockid, buf, HTTP_HEADER_LEN, 0);
	if (rd <= 0) {
		return -1;
	}
	for(int i=0;i<HTTP_HEADER_LEN;i++)
		printf("%c",buf[i]);
	printf("\n");
	...
	int sent = 0;
	while (sent < len) {
		int ret = mysend(sockid, response+sent, len-sent, 0);
		if (ret < 0) {
...
{
	int c = myaccept(listener, NULL, NULL);
	if (c >= 0) {
		if (c >= MAX_FLOW_NUM) {
...
int create_listening_socket()
{
	...
	int listener = mysocket(AF_INET, SOCK_STREAM, 0);
	if (listener < 0) {
	...
	int ret = mybind(listener, (struct sockaddr *)&saddr, sizeof(struct sockaddr_in));
	if (ret < 0) {
