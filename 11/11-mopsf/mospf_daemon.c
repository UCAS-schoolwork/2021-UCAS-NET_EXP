...
pthread_mutex_t mospf_lock; //lock for iface->nbr_list
pthread_mutex_t db_lock;    //lock for mospf_db
...
static void broadcast_lsu(const char *pkt, int len, u32 ext_rid)
{
	...
	list_for_each_entry(iface,&instance->iface_list,list){
		list_for_each_entry(nbr,&iface->nbr_list,list){
			if(nbr->nbr_id == ext_rid) continue;
			char *packet = malloc(len);
...
static void update_rtable()
{
	...
	// delete rentries that are not nbr
	rt_entry_t *rentry, *q;
	list_for_each_entry_safe(rentry,q,&rtable,list){
		if(rentry->gw)
        remove_rt_entry(rentry);
	}
	...
	list_for_each_entry(entry,&mospf_db,list){
		for(int j=0;j<entry->nadv;j++){
			...
			if(!found) //fprintf(stdout,"add arc!"IP_FMT"\n",HOST_IP_FMT_STR(entry->array[j].rid));
			{ //rid=0 will conflict
	...
	while(i2>i1){
		...
		for(int i=0;i<rnode[th].nadv;i++){
			...
			rt_entry_t *rentry = new_rt_entry(rnode[next].network,rnode[next].mask,rnode[th].gw,rnode[th].iface);
			add_rt_entry(rentry);
			vis[next] = 1;
...
void *checking_database_thread(void *param)
{
	...
	while(1){
		...
		//if(mod)
			//update_rtable();
		//Note: May conflict with rtable lookup
	}
	return NULL;
}
